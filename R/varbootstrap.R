...
#'
varbootstrap <- function(keyfn, survey, gpsdata, disttrunc=150, gradientmaxdist=100, Nboot, plotit=TRUE) {
  library(MIfuns)
  set.seed(123)
  surv.resamp <- resample.data.frame(survey,names=1:Nboot,key="Transect",rekey=T)
  real.resamp <- resample.data.frame(gpsdata,names=1:Nboot,key="animal",rekey=T)
...
#    par.estimates$EsSb[i]=par.estimates$Es[i]*size.bias.corr
    par.estimates$EsSb[i] <- size.bias(keyfn, surv.data.b.dist, surv.resamp[[i]], grad.data=gpsdata, disttrunc,
                                       gradientmaxdist, plotit=FALSE)
    if (keyfn =="HN") {
      mle2.b <- optim(par=c(50,35,35),fn=full.lik.fx.HN,x1=surv.data.b.dist,
                   x2=real.data.b.dist,w=disttrunc,w2=gradientmaxdist,control=list(fnscale=-1))#,method="L-BFGS-B",lower=c(20,-100),upper=c(200,100))
      par.estimates$sigma1[i] <- mle2.b$par[1]
      par.estimates$sigma2[i] <- mle2.b$par[2]
      par.estimates$beta[i] <- mle2.b$par[3]
    } else {
      mle2.b <- optim(par=c(50,35,35),fn=full.lik.fx.HR,x1=surv.data.b.dist,
                   x2=real.data.b.dist,w=disttrunc,w2=gradientmaxdist,control=list(fnscale=-1))#,method="L-BFGS-B",lower=c(20,-100),upper=c(200,100))
      par.estimates$sigma1[i] <- mle2.b$par[1]
